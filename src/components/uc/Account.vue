          ...
          <div class="account-in">
            <div class="account-item">
              <div class="account-item-in">
                ...
                <template v-if="user.bankVerified == 1">
                  <a class="btn" @click="showItem(1)">{{
                    $t("uc.account.modify")
                  }}</a>
                  <a class="btn" @click="unBind(1)">{{
                    $t("uc.account.unbind")
                  }}</a>
                </template>
                <a class="btn" v-else @click="showItem(1)">{{
                  $t("uc.account.bind")
                }}</a>
              </div>
              <div class="account-detail" v-show="choseItem == 1">
                <div class="detail-list">
                  ...
                  >
                    ...
                    <!-- bankName -->
                    <FormItem
                      :label="$t('uc.account.bankaccount')"
                      prop="bankName"
                    >
                    ...
                    <!-- bankBranch -->
                    <FormItem
                      :label="$t('uc.account.bankbranch')"
                      prop="bankBranch"
                    >
                      <Input
                        v-model="formValidate1.bankBranch"
                        size="large"
                      ></Input>
                    </FormItem>
                    <!-- bankNo -->
                    ...
                    <!-- bankNoConfirm -->
                    <FormItem
                      :label="$t('uc.account.confirmbankno')"
                      prop="bankNoConfirm"
                    >
                      <Input
                        v-model="formValidate1.bankNoConfirm"
                        size="large"
                        type="text"
                      ></Input>
                    </FormItem>
                    ...
                    <FormItem :label="$t('uc.account.fundpwd')" prop="password">
                      <Input
                        v-model="formValidate1.password"
                        type="password"
                        size="large"
                      ></Input>
                    </FormItem>
            ...
            <div class="account-item">
              <div class="account-item-in">
                ...
                <span class="card-number">{{
                  $t("uc.account.zfbaccount")
                }}</span>

                <p
                  v-if="user.aliVerified == 1"
                  class="bankInfo"
                  style="color: #fff;"
                >
                  {{ user.alipay.aliNo }}
                ...
                <template v-if="user.aliVerified !== 1">
                  <a class="btn" @click="showItem(2)">{{
                    $t("uc.account.modify")
                  }}</a>
                  <a class="btn" @click="unBind(2)">{{
                    $t("uc.account.unbind")
                  }}</a>
                </template>
                <a class="btn" v-else @click="showItem(2)">{{
                  $t("uc.account.bind")
                }}</a>
              </div>
                  ...
                  >
                    <Row>
                      ...
                      <Col span="16">
                        ...
                        <!-- alipay -->
                        <FormItem
                          :label="$t('uc.account.zfbaccount')"
                          prop="alipay"
                        >
                          <Input
                            v-model="formValidate2.alipay"
                            size="large"
                          ></Input>
                        </FormItem>
                        <!-- passwd -->
                        <FormItem
                          :label="$t('uc.account.fundpwd')"
                          prop="password"
                        >
                          <Input
            ...
            <div class="account-item">
              <div class="account-item-in">
                ...
                <span class="card-number">{{
                  $t("uc.account.wxaccount")
                }}</span>
                <p
                  v-if="user.wechatVerified == 1"
                  class="bankInfo"
                  style="color: #fff;"
                >
                ...
                <template v-if="user.wechatVerified == 1">
                  <a class="btn" @click="showItem(3)">{{
                    $t("uc.account.modify")
                  }}</a>
                  <a class="btn" @click="unBind(3)">{{
                    $t("uc.account.unbind")
                  }}</a>
                </template>
                <a class="btn" v-else @click="showItem(3)">{{
                  $t("uc.account.bind")
                }}</a>
              </div>
                      ...
                      <Col span="16">
                        ...
                        <FormItem :label="$t('uc.account.name')" prop="name">
                          <Input
                            disabled
                            size="large"
                            v-model="formValidate3.name"
                          ></Input>
                        </FormItem>
                        <!-- wechat -->
                        <FormItem
                          :label="$t('uc.account.wxaccount')"
                          prop="wechat"
                        >
                          <Input
                            v-model="formValidate3.wechat"
                            size="large"
                          ></Input>
                        </FormItem>
                        <!-- passwd -->
  ...
  methods: {
    unBind(val) {
      ...
      self.$Modal.confirm({
        ...
        onOk: () => {
          self.$http
            .post(self.apiHost + "/user/del_card", {
              id,
    ...
    submit(name) {
      ...
      if (name == "formValidate1") {
        let data = this.formValidate1;
        param.info = {
          bank: data.bankName,
          branch: data.bankBranch,
          jyPassword: data.password,
          realName: data.name,
          cardNo: data.bankNo,
        };
        param.type = "card";
      ...
      this.$http
        .post(this.apiHost + "/user/set_card_info", param)
        .then((response) => {
          var resp = response.body;
          if (resp.code == 1) {
      ...
      this.$http
        ...
        .then((response) => {
          ...
          if (resp.code == 1) {
            ...
            this.isNoName = false;
            //设置
            this.formValidate1.bankName =
              this.user.bankInfo == null ? "" : this.user.bankInfo.bank;
            this.formValidate1.bankBranch =
              this.user.bankInfo == null ? "" : this.user.bankInfo.branch;
            this.formValidate1.bankNo =
              this.user.bankInfo == null ? "" : this.user.bankInfo.cardNo;
            this.formValidate2.alipay =
              this.user.alipay == null ? "" : this.user.alipay.aliNo;
            this.formValidate3.wechat =
              this.user.wechatPay == null ? "" : this.user.wechatPay.wechat;
            this.aliImg = this.aliPreview =
              this.user.alipay == null ? "" : this.user.alipay.qrCodeUrl;
            this.weImg = this.wePreview =
              this.user.wechatPay == null
                ? ""
                : this.user.wechatPay.qrWeCodeUrl;
          } else {
