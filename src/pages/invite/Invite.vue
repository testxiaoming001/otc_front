<template>
  <div class="invite">
      ...
      <div class="mask" v-if="!isLogin">
        <div class="login-btn">
          <ButtonGroup size="large">
            ...
            <Button type="warning" to="/register">{{
              $t("common.register")
            }}</Button>
          </ButtonGroup>
        </div>
    ...
    <div class="main">
        ...
        <div class="content">
          ...
          >
            <p style="font-size: 13px;font-weight:bold;">
              {{ $t("invite.ruleexampletitle") }}
            </p>
            <p style="text-indent: 25px;">
              {{ $t("invite.ruleexamplecontent1") }}
            </p>
            <p style="text-indent: 25px;">
              {{ $t("invite.ruleexamplecontent2") }}
            </p>
          </div>
          ...
          <p class="rule-update">
            {{ $t("invite.lastupdate") }}：2019/07/18 12:15:34
          </p>
        </div>
      </div>
      <div class="promotion-tools">
        ...
        <div class="tools">
          <div class="item" @click="showCardItem">
            <img
              src="https://bizzan.oss-cn-hangzhou.aliyuncs.com/2019/promotioncard.png"
            />
            <p class="title">
              {{ $t("invite.pt_title")
          ...
          <div class="item" @click="useInviteImage">
            <img
              src="https://bizzan.oss-cn-hangzhou.aliyuncs.com/2019/invitebg.jpg"
            />
      ...
      <div class="invite-rank">
        <Row :gutter="80">
          <Col span="12">
            <h2>
              {{ $t("invite.totalcommissionrank") }}TOP20{{
                $t("invite.rankall")
              }}
            </h2>
            <p class="ranktitle">
          ...
          <Col span="12">
            <h2>
              {{ $t("invite.promotioncount") }}TOP20{{ $t("invite.rankall") }}
            </h2>
            <p class="ranktitle">
              <span>{{ $t("invite.laststastic") }}: {{ lastUpdate }}</span>
            </p>
      ...
      <div class="invite-rank" style="display:none;">
        <Row :gutter="80">
          <Col span="12">
            <h2>
              {{ $t("invite.promotioncount") }}TOP20{{ $t("invite.rankweek") }}
            </h2>
            <p class="ranktitle">
          ...
          <Col span="12">
            <h2>
              {{ $t("invite.promotioncount") }}TOP20{{ $t("invite.rankmonth") }}
            </h2>
            <p class="ranktitle">
              <span
                >{{ $t("invite.laststastic") }}: {{ lastStasticMonth }}</span
              >
            </p>
            <Table
              class="rank-table"
    ...
    <H1
      style="color:#000;text-align:center;margin-top: 100px;font-size:30px;;"
      >{{ $t("invite.thanks") }}</H1
    >

    <Drawer
      :title="$t('invite.pt_title')"
      :closable="false"
    ...
    >
      <div class="ptcard-header">
        ...
        <div class="desc">
          ...
          <p class="deadline">
            {{ $t("invite.pt_card_deadline") }}：60{{
              $t("invite.pt_card_day")
            }}
          </p>

          <Button
            v-if="isLogin && !hasCard"
            type="error"
            size="large"
            @click="getFreeCard"
            >{{ $t("invite.pt_card_btn") }}</Button
          >
          <Button
            v-if="isLogin && hasCard"
            type="error"
            size="large"
            @click="usePromotionImage"
            >{{ $t("invite.usepromotion") }}</Button
          >
          <Button v-else to="/login" type="success" size="large">{{
            $t("invite.pt_card_btn_login")
          }}</Button>
        </div>
      </div>
      <Divider style="margin-top: 40px;" orientation="left">{{
        $t("invite.pt_card_rule")
      }}</Divider>
      <div class="ptcard-info">
        <p class="title">{{ $t("invite.pt_card_summary") }}</p>
        <p class="detail">
          {{ $t("invite.pt_card_rule1") }}：promotion@bizzan.com。
        </p>
    ...
    >
      ...
      >
        <img
          style="width:100%;display:block;"
          src="../../assets/images/promotion/invitebg1.jpg"
        />
        <div
          class="qr-code"
        ...
        <p
          style="position:absolute;top: 375px;text-align:center;width: 100%;text-align:center;"
        >
          扫一扫了解更多
        </p>
        <p
          style="position:absolute;top: 395px;text-align:center;width: 100%;text-align:center;font-size:16px;"
        >
          我的邀请码：{{ inviteCode }}
        </p>
      </div>
...
export default {
  ...
  methods: {
    ...
    getMemberInfo() {
      this.myInfo.inviteLink =
        this.$store.state.member.promotionPrefix +
        this.$store.state.member.promotionCode;
      ...
      this.$http
        ...
        .then((response) => {
          ...
          if (resp.code == 0) {
            ...
            this.myInfo.partnerlevel = this.partnerNameByCount(
              resp.data.firstLevel
            );
          } else {
            this.$Notice.error({
